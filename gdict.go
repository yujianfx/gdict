...
func (d *Dict) dictAdd(key, value any) bool {
	if d.reHashIndex != -1 {
...
func (d *Dict) dictFind(key any) *dictEntry {
	if d.reHashIndex != -1 {
		d.dictRehash(1)
	}
...
func (d *Dict) dictDelete(key any) bool {
	if d.reHashIndex != -1 {
		d.dictRehash(1)
	}
	hash := dictHashFunction(key)
...
func (d *Dict) dictResize(size uint64) {
...
// forEach遍历字典中的所有条目并调用fn。
func (d *Dict) forEach(fn func(key any, value any)) {
	if d.reHashIndex != -1 {
		d.dictRehash(1)
	}
	for table := 0; table <= 1; table++ {
